...
from .common import clean_text, dedupe_sort, key
...
def build_resale(
    ...
    """Build normalised resale dataframes from raw resale flat prices."""
    raw_resale = raw_datasets["resale_flat_prices"]
    ...
    resale["floor_area_sqm"] = pd.to_numeric(resale["floor_area_sqm"], errors="raise")
    ...
    resale["storey_range_key"] = [
        _storey_range_key(min_s, max_s) for min_s, max_s in storey_bounds
    ]
    resale["property_key"] = (
        resale["town_key"]
        + "|"
    ...
    transactions = pd.DataFrame(
        {
            ...
            # kept for mongo; dropped before mariadb insert
            "town_key": resale["town_key"],
        }
